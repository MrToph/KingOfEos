...
if (process.env.NODE_ENV.toLowerCase() === `test`) {
  ...
  fetchHallOfFame = () => dispatch =>
    new Promise(resolve => {
      ...
      dispatch({ type: `FETCH_HALL_OF_FAME_SUCCESS`, kings })
    );
} else {
  fetchCurrentKingdom = () => dispatch =>
    getKings()
      ...
      .then(({ rows }) => {
        ...
        let kings = rows
          .map(claim => ({
            ...
            soundcloudUrl: claim.claim.song,
            kingOrder: kingdomKingIndexSplit(claim.kingdomKingIndex).kingOrder,
            kingdomOrder: kingdomKingIndexSplit(claim.kingdomKingIndex)
              .kingdomOrder,
            claimTime: new Date(claim.claimTime)
          }))
...
export const scatterClaim = ({
  ...
  return scatter
    ...
    .then(() => {
      return scateos.contract("eosio.token");
    })
    .then(contract => {
      return contract
        .transfer(accountName, `11d4fgjfdg41`, `1.0000 EOS`, memo)
        .catch(error => {
          let errorMessage = "";
          ...
          if (errorMessage.trim() === "unknown key:")
